...
use svn_subr::opt::split_arg_at_peg_revision;
use svn_subr::path::is_repos_relative_url;
...
pub enum Error {
    #[error("'{}' is just a peg revision. Maybe try '{}@' instead?")]
    BadFilename(String),
}
...
pub fn args_to_target(
    ...
    for target in known_targets {
        ...
        if is_repos_relative_url(target) {
             ...
             */
            let (true_target, peg_rev) = split_arg_at_peg_revision(target);
